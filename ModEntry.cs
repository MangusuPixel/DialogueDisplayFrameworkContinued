                ...
                e.Edit(asset =>
                {
                    ...
                    foreach (var (key, entry) in data)
                    {
                        ...
                        if (imagesWithMissingID > 0)
                            SMonitor.Log($"{key} : References {imagesWithMissingID} images{(imagesWithMissingID > 1 ? "s" : "")} with missing ID.", LogLevel.Warn);
                        ...
                        if (textsWithMissingID > 0)
                            SMonitor.Log($"{key} : References {textsWithMissingID} texts{(textsWithMissingID > 1 ? "s" : "")} with missing ID.", LogLevel.Warn);
